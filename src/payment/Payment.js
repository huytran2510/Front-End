  ...
  const handleCouponValidation = async () => {
    try {
      const response = await ajax(
        "api/discount/validate", // URL endpoint
        null, // JWT (nếu không có, để null)
        "POST", // Request method
        {
      ...
      if (response && response.valid) {
        setResponseMessage(response.discountAmount);
      } else {
        setResponseMessage(response.message || "Mã khuyến mãi không hợp lệ.");
      }
    } catch (error) {
      setResponseMessage(`Lỗi khi kết nối API: ${error.message}`);
    }
  };
  ...
  const handlePayment = async () => {
    try {
      ...
      if (selectedMethod === "ZaloPay") {
        endpoint = `http://localhost:5000/zalopay/payment/${totalPriceCart}`;
      } else if (selectedMethod === "MoMo") {
        endpoint = `http://localhost:5000/momo/payment/${totalPriceCart}`;
      } else if (selectedMethod === "COD") {
        endpoint = `http://localhost:9091/api/place-order`;
        payload = {
          order: {
      ...
      if (selectedMethod === "COD" && response.status === 200) {
        alert(
          "Đơn hàng của bạn đã được đặt thành công! Cảm ơn bạn đã sử dụng dịch vụ của chúng tôi."
        );
        ...
        if (redirectUrl) {
          localStorage.setItem(
            "info",
            JSON.stringify({ name, phone, email, note, selectedAddress })
          );
          ...
          throw new Error(
            `Không tìm thấy URL thanh toán cho ${selectedMethod}`
          );
        }
      } else {
      ...
      <div className="container-lg container-fluid custom-checkout mt-5">
        <div className="row justify-content-center">
          ...
          <div className="ml-lg--50 d-block">
            ...
            <div className="d-none tch-checkout-box tch-checkout-box--lg-shadowbox d-lg-block float-lg-right pb-0 pt-0">
              <div className="ptl-20">
                <div className="tch-checkout-box tch-checkout-box--list-ordered tch-checkout-border w-100">
                  <div
                    data-v-d1bcb9c8=""
                  ...
                  {cartItem.length === 0 ? (
                    <div className="cart-empty container d-flex flex-column justify-content-center align-items-center">
                ...
                >
                  ...
                  >
                    ...
                    <div data-v-d1bcb9c8="" class="tch-order-card__right">
                      <p data-v-d1bcb9c8="" class="tch-order-card__price mb-0">
                        {responseMessage ? (
                          <p>{responseMessage.toLocaleString()}đ</p>
                        ) : (
                          <p>0đ</p>
                        )}
                      </p>
